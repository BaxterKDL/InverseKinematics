...
class FKSolver:
    ...
    def convert2T(self, pose):
        ...
        RotZ = np.matrix([[np.cos(a), -np.sin(a), 0, 0],
        ...
        RotY = np.matrix([[np.cos(b), 0, np.sin(b), 0],
        ...
        posi = np.array([pose['x'],pose['y'],pose['z'],1])

        T[0:4,3] = posi.reshape(4,1)
    ...
    def compare_pose(self, joint1, joint2):

        min_joint1 = min_joint2 = 350.5

        for joint in joint1.keys():

            min_joint1 = min(min_joint1,min(abs(self.robot.angle_limits[joint][1] - joint1[joint]),abs(joint1[joint] - self.robot.angle_limits[joint][0])))

            min_joint2 = min(min_joint1,min(abs(self.robot.angle_limits[joint][1] - joint2[joint]),abs(joint2[joint] - self.robot.angle_limits[joint][0])))

        if min_joint1 < min_joint2:
